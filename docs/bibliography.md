...
|---|---|---|
| `;lnet`, `chat/` | [lnet.lic](https://github.com/elanthia-online/scripts/blob/master/scripts/lnet.lic) (Lich's LNet client, in elanthia-online/scripts) and the [LNet server](https://lnet.lichproject.org) | The `;chat` / `;reply` / `;who` grammar is a 1:1 port of lnet.lic; the login handshake, ping replies and pinned CA follow the reference client. Replies to `;who` and `;stats` are Ruby Marshal, read by `chat/rmarshal.py`. |
...
| `;clock`, `client/eltime.py`, the Clocks dock | [Elanthipedia: Time](https://elanthipedia.play.net/Time), [Xibar](https://elanthipedia.play.net/Xibar), [Yavash](https://elanthipedia.play.net/Yavash), [Katamba](https://elanthipedia.play.net/Katamba) | The Elanthian calendar, anlas, and the moons' periods. See [eltime.md](eltime.md). |
