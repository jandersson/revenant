...
| gem_pouch | finds are tried into the pouch first; what the pouch refuses is stowed like loot. What a SEARCH leaves on the ground is read off the room's listing before and after it (`client/game/loot.py`, after dr-scripts' combat-trainer, 2026-09-23), whatever the game called it: coins, gems and boxes are what a hunt takes — plus `loot_additions`, less `loot_subtractions`, at most `box_limit` boxes — each with one STOW and its answer read (a gem into the named pouch instead); another hunter's loot ("is not yours") is left, a noun the game finds no room for is left and not tried again that run, and nothing is dropped. The answer's own wording ("You find a small ruby.", "The grendel was carrying some waermodi stones, ...") is the second look |
| loot_additions, loot_subtractions, box_limit | nouns a hunt also picks up, nouns it never does, and how many boxes it carries at most (0: no limit). A loot container that refuses a box ends box pickups for the run and a boxes farm with them: STOW BOX leaves the refused box in hand ("You pick up a reinforced oaken chest. There isn't any more room in the sack for that.", 2026-09-26), and a later box's GET once sheathed the weapon to free a hand for it; `;boxes` puts a box in hand away before its run, or works it first |
