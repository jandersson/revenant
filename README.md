...
- **Compass, Clocks, Thoughts, Arrivals, Deaths, Group** — exits you can click, Elanthian date and moons beside Stockholm and Chicago, and the game's side streams each in a window of their own. Arrivals fills only once the character's `FLAG LOGON ON` is set in the game (off by default, per character).
