...
A word on where this sits. Revenant is a hobby, built for fun and for the author's own play, not a replacement for anything. It is inspired by Lich and the community's scripts — the session-and-scripts split, the `;` commands, the community map — and exists because nothing like them existed for Python; the [Friends](#friends) section credits what it leans on. The client, the GUI, the LNet chat client and beholder were written and working by hand first — the history runs from 2018, and those pieces predate any AI involvement by years. Since spring 2026 much of the new code is written with an AI assistant (Claude Code), in the open: the commit history says which commits. It accelerates the delivery; the project, its direction and its taste are the author's. If any of it is useful to you, take it; if it isn't, the projects it learned from are a click away.
