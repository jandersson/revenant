...
- **Compass, Clocks, Thoughts, Arrivals, Deaths, Group, Attention** — exits you can click, Elanthian date and moons beside Stockholm and Chicago, the game's side streams each in a window of their own, and what `;sentinel` flags as new or addressed at you while you are away. Arrivals fills only once the character's `FLAG LOGON ON` is set in the game (off by default, per character).
...
File → Settings covers the font, the autostarts, the room ids, and whether closing the window quits the game. Highlight patterns of your own live in `~/.revenant/highlights.json`; three soft ones ship by default (the combat balance line, the roundtime line and the spell-ready lines), and an entry `{"disable": "balance"}` in that file turns one off. Scripts never drop an item; the only exceptions are the foraged junk `;mechlore` braids and whatever `droppable` in `~/.revenant/settings.json` names.

Bundled scripts: `;go2` travel on the community map, `;xp` and `;sheet` history logging, `;deathwatch` for unattended deaths, `;tend` for bleeders, `;heal` (eat the herb that treats each HEALTH wound, buying what is missing), `;clock` for the Elanthian calendar, `;circle` for what gates your next circle, `;athletics`, `;attune` (power walking), `;forage` (COLLECT rock until Outdoorsmanship locks, Perception riding along), `;research` (a Barbarian's MEDITATE RESEARCH until Augmentation, Warding and Utility lock), `;tdp`, `;debt`, `;skins` (sell the worn skin bundle at the nearest tannery), `;repair` (appraise your gear and have the worn-down pieces mended at the nearest repair shop), `;enc`, `;status`, `;favors`, `;mechlore`, `;wealth`, `;survey`, `;hunt` (a hunting loop driven by a per-character profile, File → Character Profile…: weapon and stance, skins onto a worn bundle, buffs kept up and recast to train a magic skill, health and wound floors), `;train` (the orchestrator: runs the trainers a per-character plan names, File → Training Plan…, until each skill's mindstate fills, then rests in a safe room until it drains), and `;lnet`. `;help` lists them with their manuals.
...
- [movement.md](docs/movement.md), [death.md](docs/death.md), [favors.md](docs/favors.md), [circles.md](docs/circles.md), [experience.md](docs/experience.md), [training.md](docs/training.md), [healing.md](docs/healing.md), [social.md](docs/social.md), [encumbrance.md](docs/encumbrance.md), [eltime.md](docs/eltime.md), [combat.md](docs/combat.md), [soul.md](docs/soul.md), [wounds.md](docs/wounds.md) — the game models the scripts implement, each with its captured evidence; [paladin.md](docs/paladin.md) and [barbarian.md](docs/barbarian.md) — a guild's measured facts indexed in one place, the model per fact linked
